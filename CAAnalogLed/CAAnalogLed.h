 ...
 * CAAnalogLed class.
 *
 * This is a base class that handles common anode (CA) LED light 
 * analog (PWM) output on the Arduino Uno. Instead of relying to 
 * delay() to control activities such as blinking and fading, 
 * it uses the change in  millis() between the loop() function 
 * calls and the specified interval to calculate the brightness 
 * change amount during each loop.
 * 
 * @author Janette H. Griggs
 * @version 1.0 04/04/14
 */

#ifndef CAAnalogLed_h
  #define CAAnalogLed_h
...
class CAAnalogLed {
  public:
     ...
     * at least 0.
     */
    CAAnalogLed(int ledPinNumber, int minBrightness, int maxBrightness);
